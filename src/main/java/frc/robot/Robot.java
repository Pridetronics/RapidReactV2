...
public class Robot extends TimedRobot {
  ...
  SendableChooser<Command> chooser = new SendableChooser<Command>(); // Creates sendable chooser (allows us to choose
                                                                     // autonomous programs)
  ...
  public void robotInit() {
    /*
     * Instantiate RobotContainer. This will allow access to hardware and
     * subsystems. BUT the only commands that can be run
     * are those called in autonomous
     */
    m_robotContainer = new RobotContainer();
    SmartDashboard.putString("Program:", "Reorgnized Code"); // This is meant to help identify the code, be sure to
                                                             // change the string
    // The next three lines set the default state for both pistons and the servo.
    // Called upon initialization (when robot is enabled)
    ...
    /*
     * Fully complex autonomous. This shoots the preloaded ball (shooter runs for 2
     * seconds) [stage 1],
     * leaves the tarmac [stage 2] runs the intake for four seconds (grabbing the
     * ball) [stage 3]. Then reenters
     * the tarmac, waits one second [stage 4], and then shoots the ball within two
     * seconds [stage 5]. Then quickly
     * leaves the tarmac for the autonomous points (leaving tarmac + balls being
     * shot) [stage 6 and fin]
     */
  ...
  public void autonomousInit() {
    // When running autonomous command, it gets the command that is selected by the
    // user.
