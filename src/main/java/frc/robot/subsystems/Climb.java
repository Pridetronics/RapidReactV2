...
public class Climb extends SubsystemBase {
    ...
    public void periodic() {
        if (true) { // True will enable -- (If false, won't show on Smart Dashboard, meant for
                    // resource management)
            ...
            SmartDashboard.putNumber("Climb Encoder", RobotContainer.climbEncoder.getPosition()); // Measures in units
                                                                                                  // of rotations by
                                                                                                  // default
        }
    }
    ...
    /*
     * zeroEncoder: Sets encoders to 0, essential with climb. If the encoders were
     * not zeroed before the climb
     * sequence was run, there'd be a potential for it to misread the values and
     * overextend (therefore
     * breaking the climb). When using encoders, especially with position PID, you
     * want to ensure that
    ...
    /*
     * extendPivotArmDistanceOne: Uses position control to reach a certain height
     * for the first
     * stage of climb. Disclaimer, the numbers are not precise and were found
     * through testing. If
     * we figure out where the numbers come from, this will be removed. EXPLAIN WHAT
     * CLIMBGOAL IS.
     */
    ...
    // extendPivotArmDistanceTwo: Uses position control to reach a certain height
    // for the second stage of climb.
    ...
    // extendPivotArmDistanceThree: Uses position control to reach a certain height
    // for the third stage of climb.
    ...
    // descendPivotArmDistance: Uses position control to reach a certain height
    // (negative), to pull up the robot.
    ...
    // This is the inches per revolutions conversion. This is used for PID. It is
    // not very accurate. It is off by x5.
    ...
    // isClimbAtTop: Soft limit switch to ensure that the limit switch does not run
    // too high.
    ...
    /*
     * isClimbAtBottom: Checks the activation of the limit switch (hardware) and
     * returns a true
     * or false. This looks a little contradictory, and because of the nature of the
     * hardware
     * this exists as a wrapper class to make it readable for the programmer. While
     * the hardware
     * reads it the same, it is much easier to know that the limit switch is active
     * == true, as
    ...
    /*
     * Also for climb honing. If the limit switch is not hit, the motor will run at
     * negative 10%
     * until it hits the limit switch before zeroing. Ensures the proper position is
     * available
     * for the climb sequence.
     */
