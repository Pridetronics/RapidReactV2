...
public class Climb extends SubsystemBase {
  ...
  * sequence was run, there'd be a potential for it to misread the values and overextend (therefore
  ...
  */
  ...
  /* 
  *  extendPivotArmDistanceOne: Uses position control to reach a certain height for the first 
  *  stage of climb. Disclaimer, the numbers are not precise and were found through testing. If 
  *  we figure out where the numbers come from, this will be removed. EXPLAIN WHAT CLIMBGOAL IS.
  */
  ...
  //pistonRelease: Moves piston WHICH DIRECTION
  public void pistonRelease() {
    m_climbPiston.set(DoubleSolenoid.Value.kForward);
  }

  //pistonRetract: Moves piston WHICH DIRECTION
  ...
  */
  ...
  /* 
  *  Used for climb honing. ClimbUpSlowly sets the motor to 10% and if the limit switch is active
  *  it will raise it off the switch and zero the encoder (creating the starting position)
  */
  ...
  /*
  *  Also for climb honing. If the limit switch is not hit, the motor will run at negative 10%
  *  until it hits the limit switch before zeroing. Ensures the proper position is available 
