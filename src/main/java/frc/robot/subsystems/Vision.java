...
public class Vision extends SubsystemBase {
  ...
  // The following is all involved in Limelight values. Explained further within
  // functions
  private double ty;
  private double tx;
  private double tv;
  ...
  public Vision() {
    ...
    m_shooterMotor = RobotContainer.shooterLeftMotor;
  ...
  public void periodic() {
    // While there are comments, this is a tricky topic-- check out the
  ...
  // lightsOut: Turns out LEDs, was created to be called in Robot Init (so the
  // light wouldn't be on until needed)
   ...
   * Processing mode prepares it for use with shooter/drive. CameraMode is solely
   * for user vision
   */
  public void processingMode() {
    NetworkTableInstance.getDefault().getTable("limelight").getEntry("camMode").setNumber(0); // Sets camMode to
                                                                                              // processing
  ...
  public void cameraMode() {
    NetworkTableInstance.getDefault().getTable("limelight").getEntry("camMode").setNumber(1); // Sets camMode to camera
                                                                                              // vision
    NetworkTableInstance.getDefault().getTable("limelight").getEntry("ledMode").setNumber(1); // Forces LEDs off.
  }

  /*
   * Seek Target: Checks if the Limelight has a target, if it does not, it will
   * utilize PID control to
   * adjust until it finds a target. PID control is for movement speed. This is
  ...
  /*
   * TargetAdjust: Meant to center the robot on the horizontal axis, uses the
   * drive train to turn slightly
   * based on where it falls within the angles of vision. If the target is too far
   * right, it will turn left.
  ...
  // findDistance: Calculates distance and converts it into rounded distance for
  // use within the automatic shooter code
  ...
  public void AutomaticShooter() {
    if (roundedDistance > 13) // This if statement checks for the distance (see find distance) and picks RPM
                              // based on this
    {
      // This line sets the PID controller to listed RPM (first number), add control
      // type so controller knows which value is being impacted
  ...
  // AutomaticOpenGate: Checks for a velocity provided by automaticshooter to
  // check if open gate conditions are met (then opens gate)
