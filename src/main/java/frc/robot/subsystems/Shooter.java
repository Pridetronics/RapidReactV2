...
public class Shooter extends SubsystemBase {
  ...
  public Shooter() {
    // Motor, Encoder, PID, and Servo referenced from RobotContainer for use in
    // subsytem.
  ...
  // LowSpeedShooter: Sets shooter motor to 2500 RPM. Also displays this on
  // SmartDashboard
  public void LowSpeedShooter() {
    /*
     * The line below sets the speeed for the motor using a PID loop. The first
     * value is the RPM desired (human input),
     * and the second number is the ControlType (in this case velocity), which tells
     * the encoder (Position is the other
     * type-- see climb)
     */
  ...
  // HightSpeedShooter: Sets shooter motor to 5000 RPM and displays on
  // SmartDashboard
  ...
  public void ShooterStop() {
    ...
    // Set measures in voltage-- power outputted to the motor. In this case, 0.
    // (Range = -1 to 1)
  }

  /*
   * OpenGateLow checks for a certain velocity (2500 in this case) and if the
   * conditions are met,
   * moves the servo gate to a position in which the ball can move through. Both
   * open gate commands
   * ensure the motor is able to reach the proper speed so the ball is launched
   * properly.
   */
  public void OpenGateLow() {
    m_shooterServo.setRaw(Constants.shooterServoOpenPosition); // This is measured in ms-- see Pridetronics
                                                               // documentation for more information

  }

  /*
   * OpenGateHigh checks for a certain velocity (5000 in this case) and if the
   * conditions are met,
  ...
  // CloseGate: Moves servo to a position in which the ball is unable to be
  // released.
